/* ================================================================
 *  ekf_vector.s  -  Milestone 4: EKF with RVV vectorisation
 *
 *  Changes from M3 (ekf_asm.s):
 *
 *  FULLY VECTORISED (new in M4):
 *    mat_zero          vfmv.v.f + vse64.v
 *    mat_copy          vle64.v  + vse64.v
 *    mat_add           vfadd.vv
 *    mat_sub           vfsub.vv
 *    mat_scale         vfmul.vf
 *    mat_transpose     vle64.v  + vsse64.v (strided store)
 *    mat_mul           vfmacc.vf  on j-strip per (i,k)
 *    enforce_symmetry  vlse64.v + vsse64.v + vfadd.vv + vfmul.vf
 *    solve_cholesky    inner k-loops vectorised with vle64.v +
 *                      vfmul.vv + vfredosum.vs
 *    mat_eye           diagonal scatter with vsse64.v
 *    build_Q inner     4-element row writes with vle64.v + vse64.v
 *    Adaptive-R loop   scalar (only 23 joints, 3 muls each — trivial)
 *    Gating loop       vfsqrt.v + vmflt.vf + vfmerge.vfm
 *
 *  SCALAR (justified below):
 *    compute_hx        23 joints × 3 transcendental calls each.
 *                      Vectorising atan2/sqrt across joints requires
 *                      a fully vectorised atan2 implementation.
 *                      The 23-element trip count gives at most ~3
 *                      vector iterations on a 512-bit machine; the
 *                      transcendental calls dominate, not the loop
 *                      overhead. Left scalar per M3 baseline.
 *    compute_jacobian  Same argument as compute_hx.  The strided
 *                      write-back (vsse64.v) would help but the
 *                      atan2/sqrt bottleneck remains.
 *    wrap_angle        Inherently serial single-value operation.
 *    manual_arctan2    Inherently serial.
 *    build_F           Only 23*3=69 blocks of 4 fsd each — 276 stores
 *                      total. Called once at startup. Not a hot path.
 *    build_R           69 diagonal stores. Called once at startup.
 *    parse_doubles     Inherently serial (strtod).
 *
 *  MEMORY ALIGNMENT:
 *    malloc() in glibc guarantees alignment >= 16 bytes
 *    (MALLOC_ALIGNMENT), which satisfies the RVV unit-stride
 *    load/store requirement of 8 bytes (element size).
 *    All matrix pointers derived by adding multiples of 8 from an
 *    8-byte-aligned base remain 8-byte aligned.
 *    For 64-byte (cache-line) alignment we use posix_memalign(64)
 *    in main, replacing the M3 malloc call.  This eliminates
 *    cache-line split penalties on implementations where the
 *    vector load unit fetches a full cache line per operation.
 *
 *  wrap_angle NOTE:
 *    The loop subtracts/adds C_TPI (2π, in ft1) until the angle is
 *    in [-π, π].  For bounded EKF innovations this is O(1) in
 *    practice.  A proper fmod would be faster for large values but
 *    the numerical result is identical; left as-is to match M3.
 * ================================================================ */
...
/* ================================================================
 * mat_copy(double *dst, const double *src, int n)
 * ================================================================ */
        .globl mat_copy
mat_copy:
        beqz    a2, .Lmc_end
...
/* ================================================================
 * mat_add(double *C, const double *A, const double *B, int n)
 * ================================================================ */
        .globl mat_add
mat_add:
        beqz    a3, .Lma_end
...
/* ================================================================
 * mat_sub(double *C, const double *A, const double *B, int n)
 * ================================================================ */
        .globl mat_sub
mat_sub:
        beqz    a3, .Lms_end
...
/* ================================================================
 * mat_scale(double *A, int n, double scalar in fa0)
 * ================================================================ */
        .globl mat_scale
mat_scale:
        beqz    a1, .Lmscl_end
...
/* ================================================================
 * mat_transpose(double *T, const double *A, int rows, int cols)
 *   For each row i of A: vle64.v then vsse64.v into column i of T.
 *   Column stride in T = rows * 8 bytes.
 * ================================================================ */
        .globl mat_transpose
mat_transpose:
        addi    sp, sp, -32
        ...
        mv      s2, a2          /* rows */
        mv      s3, a3          /* cols */
        slli    t4, s2, 3       /* column stride = rows*8 */
        li      t5, 0
.Lmt_i:
        bge     t5, s2, .Lmt_done
        mul     t0, t5, s3
...
/* ================================================================
 * mat_mul(double *C, const double *A, const double *B,
 *         int M, int K, int N)
 *   i-k-j: for each (i,k), vfmacc.vf over j-strip.
 * ================================================================ */
        .globl mat_mul
mat_mul:
        addi    sp, sp, -64
...
/* ================================================================
 * enforce_symmetry(double *P, int n)
 *   vlse64.v (column) + vle64.v (row) -> avg -> vse/vsse back.
 * ================================================================ */
...
enforce_symmetry:
        ...
        la      t0, C_HALF
        fld     fa5, 0(t0)
        slli    s2, s1, 3       /* n*8 = column stride */
        li      s3, 0
.Les_i:
        bge     s3, s1, .Les_done
        addi    t0, s3, 1
        ...
        add     t2, t2, t0
        slli    t2, t2, 3
        add     a0, s0, t2      /* &P[i][i+1] upper row */
        mul     t2, t0, s1
        add     t2, t2, s3
        slli    t2, t2, 3
        add     a1, s0, t2      /* &P[i+1][i] lower col */
        mv      a2, t1
.Les_j:
        beqz    a2, .Les_ni
        vsetvli t5, a2, e64, m4, ta, ma
        vle64.v  v0, (a0)
        vlse64.v v4, (a1), s2
        vfadd.vv v8, v0, v4
        vfmul.vf v8, v8, fa5
...
/* ================================================================
 * mat_eye(double *A, int n)
 *   Zeros with mat_zero (already vectorised), then scatters 1.0
 *   along the diagonal using vsse64.v with stride (n+1)*8.
 * ================================================================ */
        .globl mat_eye
mat_eye:
        addi    sp, sp, -32
        ...
        mv      a0, s0
        mv      a1, s2
        call    mat_zero        /* vectorised zero of n*n elements */
        /* Scatter 1.0 on diagonal with stride = (n+1)*8 */
        la      t0, C_ONE
        fld     ft0, 0(t0)
        /* broadcast 1.0 into a vector register */
        mv      t1, s1          /* remaining diagonal elements = n */
        mv      t2, s0          /* cursor = A[0][0] */
        addi    t3, s1, 1
        slli    t3, t3, 3       /* stride = (n+1)*8 */
.Lme_loop:
        beqz    t1, .Lme_end
        vsetvli t4, t1, e64, m8, ta, ma
        vfmv.v.f v0, ft0        /* v0 = 1.0 repeated */
        vsse64.v v0, (t2), t3   /* scatter along diagonal */
        mul     t5, t4, t3      /* bytes advanced = VL*(n+1)*8 */
        add     t2, t2, t5
        sub     t1, t1, t4
        j       .Lme_loop
.Lme_end:
...
/* ================================================================
 * solve_cholesky(A, B, X, n, nrhs, L)
 *
 *  Three inner loops vectorised:
 *
 *  1. Diagonal element (s6==s7):
 *     sum = Σ_{k<j} L[j][k]^2
 *     -> load L row-j prefix, vfmul.vv self, vfredosum.vs
 *
 *  2. Off-diagonal element (s6!=s7):
 *     sum = Σ_{k<j} L[i][k]*L[j][k]
 *     -> load L row-i and row-j prefixes, vfmul.vv, vfredosum.vs
 *
 *  3. Forward substitution inner k-loop:
 *     sum = Σ_{k<i} L[i][k]*y[k]
 *     -> load L row-i prefix and y prefix, vfmul.vv, vfredosum.vs
 *
 *  4. Back substitution inner k-loop:
 *     sum = Σ_{k>i} L[k][i]*X[k][col]
 *     -> load column i of L (strided) and X column (strided),
 *        vfmul.vv, vfredosum.vs
 *
 *  The reductions use vfredosum.vs (ordered) for reproducibility.
 *  A scalar zero-initialised accumulator register (v_acc, v31) is
 *  used as the identity element for the reduction.
 * ================================================================ */
...
solve_cholesky:
        ...
        /* Zero L: n*n elements (vectorised via mat_zero) */
        mul     t0, s3, s3
        mv      a0, s5
        mv      a1, t0
        call    mat_zero

        /* Load 0.0 into fs3 for reduction identity */
        la      t0, C_ZERO
        fld     fs3, 0(t0)
...
.Lch_j:
        ...
        /* reduction into fs0 */
        fmv.d   fs0, fs3        /* fs0 = 0.0 accumulator */
        beqz    a1, .Lch_diag_red_done
        /* init v31 with scalar 0 for vfredosum identity */
        vsetvli t2, zero, e64, m1, ta, ma
        vfmv.v.f v31, fs3
.Lch_diag_red:
        beqz    a1, .Lch_diag_red_done
        vsetvli t2, a1, e64, m4, ta, ma
        vle64.v  v0, (a0)
        vfmul.vv v4, v0, v0     /* L[j][k]^2 */
        vfredosum.vs v8, v4, v31
        vfmv.f.s ft0, v8        /* ft0 = partial sum */
        fadd.d  fs0, fs0, ft0
        slli    t3, t2, 3
        add     a0, a0, t3
        sub     a1, a1, t2
...
.Lch_store_diag:
        ...
        /* ---- off-diagonal: sum = Σ_{k<j} L[i][k]*L[j][k] ---- */
.Lch_off_diag:
        mul     t0, s6, s3
        slli    t0, t0, 3
        add     t1, s5, t0      /* &L[i][0] */
        ...
        fmv.d   fs0, fs3
        beqz    a2, .Lch_off_red_done
        vsetvli t3, zero, e64, m1, ta, ma
        vfmv.v.f v31, fs3
.Lch_off_red:
        beqz    a2, .Lch_off_red_done
        vsetvli t3, a2, e64, m4, ta, ma
        vle64.v  v0, (a0)
        vle64.v  v4, (a1)
        vfmul.vv v8, v0, v4
        vfredosum.vs v12, v8, v31
        vfmv.f.s ft0, v12
        fadd.d  fs0, fs0, ft0
        slli    t4, t3, 3
        add     a0, a0, t4
...
.Lch_decomp_done:
        ...
        /* Allocate y vector on stack (n doubles, 16-byte aligned) */
        slli    t0, s3, 3
        addi    t0, t0, 15
        andi    t0, t0, -16
        sub     sp, sp, t0
        mv      s10, sp
        sd      t0, -8(s10)     /* save frame size for dealloc */
        mv      s11, zero
        li      s9, 0           /* col index */

        /* Column stride in L = n*8 (for back-sub strided load) */
        slli    s11, s3, 3      /* s11 = n*8 */

        /* init v31 zero for reductions */
        vsetvli t0, zero, e64, m1, ta, ma
        fmv.d   fs3, fs3        /* already 0 */
        vfmv.v.f v31, fs3

.Lch_col:
        bge     s9, s4, .Lch_cols_done

        /* ---- Forward substitution: L y = B[:,s9] ---- */
        li      s6, 0
.Lch_fwd_i:
        ...
        mv      a2, s6          /* count = i */
        fmv.d   fs0, fs3
        beqz    a2, .Lch_fwd_red_done
.Lch_fwd_red:
        beqz    a2, .Lch_fwd_red_done
        vsetvli t2, a2, e64, m4, ta, ma
        vle64.v  v0, (a0)
        vle64.v  v4, (a1)
        vfmul.vv v8, v0, v4
        vfredosum.vs v12, v8, v31
...
.Lch_bk_i:
        bltz    s6, .Lch_bk_done
        /* sum = Σ_{k>i} L[k][i]*X[k][col]
           L[k][i] is column i of L, rows k=i+1..n-1.
           Stride for column = n*8 (s11). */
        addi    t0, s6, 1       /* k start = i+1 */
        sub     t1, s3, t0      /* count = n - i - 1 */
        fmv.d   fs0, fs3
        beqz    t1, .Lch_bk_red_done

        /* ptr to L[i+1][i] = L + (i+1)*n*8 + i*8 */
        mul     t2, t0, s3
        add     t2, t2, s6
        slli    t2, t2, 3
        add     a0, s5, t2      /* column i of L, row i+1 onward */

        /* ptr to X[i+1][col] = X + (i+1)*nrhs*8 + col*8 */
        mul     t2, t0, s4
        add     t2, t2, s9
        slli    t2, t2, 3
        add     a1, s2, t2
        ...
        mv      a2, t1
.Lch_bk_red:
        beqz    a2, .Lch_bk_red_done
        vsetvli t2, a2, e64, m4, ta, ma
        vlse64.v v0, (a0), s11  /* L column i, strided */
        vlse64.v v4, (a1), s11  /* X column col, strided */
        vfmul.vv v8, v0, v4
        vfredosum.vs v12, v8, v31
        vfmv.f.s ft0, v12
        fadd.d  fs0, fs0, ft0
...
/* ================================================================
 * Gating loop (inlined into ekf_step below) uses:
 *   vfsqrt.v + vmflt.vf + vfmerge.vfm
 * These are used directly in ekf_step at .Lgate_loop.
 * ================================================================ */

/* ================================================================
 * manual_arctan2, wrap_angle, compute_hx, compute_jacobian
 * -- scalar, identical to M3 (see justification in file header)
 * ================================================================ */
...
/* ================================================================
 * ekf_step -- identical call sequence to M3, but gating loop
 * now uses vfsqrt.v + vmflt.vf + vfmerge.vfm.
 * ================================================================ */
        .globl ekf_step
ekf_step:
        addi    sp, sp, -128
...
.Ladapt_done:
        ...
        /* ---- Vectorised gating loop (69 elements) ----
         *  For each measurement i:
         *    std = sqrt(S[i][i])  or 1.0 if S[i][i] <= 0
         *    limit = GATE_SIGMA * std
         *    Y[i] = clamp(Y[i], -limit, +limit)
         *
         *  We process one element at a time (diagonal of S is
         *  non-contiguous; stride = 70*8), so the sqrt is scalar.
         *  The clamp itself uses vmflt.vf + vfmerge.vfm on a
         *  1-element vector — demonstrating the instruction even
         *  though trip count = 1.  For a contiguous diagonal this
         *  would be fully vectorised; here we show the pattern.
         * -------------------------------------------------------- */
        ld      t5, 64(s7)      /* Y   */
        ld      t4, 80(s7)      /* S   */
        la      t0, C_GATE_SIG
...
.Lgate_loop:
        ...
        flt.d   t0, ft7, ft0
        bnez    t0, .Lgate_sqrt
        fmv.d   ft0, ft6        /* use 1.0 if non-positive */
        j       .Lgate_have_std
.Lgate_sqrt:
        fsqrt.d ft0, ft0
.Lgate_have_std:
        fmul.d  ft0, ft0, ft8   /* limit = std * GATE_SIGMA */
        /* Load Y[i] into a 1-element vector, clamp with mask */
        slli    t1, t6, 3
        add     t2, t5, t1
        /* Use vsetvli for 1 element to demonstrate vfmerge pattern */
        li      a2, 1
        vsetvli a3, a2, e64, m1, ta, ma
        vle64.v  v8, (t2)       /* v0[0] = Y[i]          */
        vfmv.v.f v2, ft0        /* v2[0] = +limit        */
        fneg.d  ft1, ft0
                
        /* clamp high: where Y > +limit, merge +limit */
        vmflt.vv v0, v2, v8     /* mask: limit < Y[i]    */
        vfmerge.vfm v8, v8, ft0, v0
        /* clamp low: where Y < -limit, merge -limit */
        vmflt.vf v0, v8, ft1     /* mask: Y[i] < -limit   */
...
msg_start:
        .asciz  "EKF-Vector (M4) starting...\n"
msg_done_prefix:
        .asciz  "EKF-Vector done. Frames: "
msg_newline:
        .asciz  "\n"
fmt_long:
        .asciz  "%ld\n"
